...
export const ScreenerChat: React.FC<Props> = ({ onSelectTicker }) => {
  ...
  return (
    <div className="w-full h-[450px] flex flex-col neo-card-flat bg-slate-50 border-2 border-black overflow-hidden">
      {/* Header */}
      <div className="bg-neo-purple border-b-[3px] border-black p-2 flex items-center gap-2 shrink-0">
        <div className="w-8 h-8 rounded-full border-2 border-black bg-white flex items-center justify-center font-black text-sm shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
          🤖
        </div>
        <div>
          <h2 className="font-black text-black uppercase tracking-tight text-sm leading-none">NeoQuant Screener</h2>
          <span className="text-[9px] font-bold uppercase tracking-widest text-black/70">AI Search</span>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3">
        {messages.map((msg) => (
          <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[90%] border-2 border-black p-2 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] ${
              msg.role === 'user' ? 'bg-neo-lime' : 'bg-white'
            }`}>
              <p className="font-bold text-black text-xs whitespace-pre-wrap">{msg.content}</p>
                  ...
                  <div className="flex flex-col gap-2">
                    {msg.tickers.map(sym => {
                      ...
                      return info ? (
                        ...
                        >
                          <div>
                            <div className="font-black text-xs uppercase text-black">{info.name.split(' ')[0]}</div>
                            <div className="text-[9px] font-bold text-slate-600">{info.symbol}</div>
                          </div>
                          <span className="font-black text-[10px] px-1 py-0.5 bg-black text-white group-hover:bg-white group-hover:text-black transition-colors">
                            ↗
                          </span>
                        </button>
                      ) : null;
      ...
      {/* Input Area */}
      <div className="p-2 bg-white border-t-[3px] border-black shrink-0">
        <form onSubmit={handleSubmit} className="flex gap-2">
          <input
            type="text"
            value={inputValue}
            onChange={(e) => setInputValue(e.target.value)}
            placeholder="Message NeoQuant..."
            className="flex-1 neo-input text-xs p-2 min-w-0"
            disabled={isLoading}
          />
          <button
            type="submit"
            disabled={isLoading || !inputValue.trim()}
            className="neo-btn bg-black text-white hover:bg-neo-yellow hover:text-black px-3 py-2 text-xs flex-shrink-0"
          >
            {isLoading ? '...' : 'SEND'}
          </button>
        </form>
